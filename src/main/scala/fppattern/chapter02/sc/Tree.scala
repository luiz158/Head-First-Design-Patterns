package fppattern.chapter02.sc
  ...
  case class TreeNode(v: Int,
    ...
    def insert(av: Int): Option[TreeNode] = {
      def insertLeft(av: Int) =
        // if left is None, the entire expression results in a None or else kicks into action —a new TreeNode is created and put into an Option and returned.
        left.flatMap(_.insert(av)) orElse Option(TreeNode(av)) // 1

      def insertRight(av: Int) =
        right.flatMap(_.insert(av)) orElse Option(TreeNode(av)) // 2
